...
- [[docs/cli/build|build]]: Build your Quartz site into static HTML. Includes a development server.
...
Commands that don't perform parallel work accept `-c` as a no-op, so it's always safe to pass. See [[docs/cli/build#Performance Tuning|build]] and [[cli/plugin#Installing on low-end hardware|plugin]] for practical examples.
